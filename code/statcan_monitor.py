...
STATE_FILE = "statcan_state.json"
DATA_DIR = "data"
STATCAN_XLSX = os.path.join(DATA_DIR, "statcan_windsor_area_monthly.xlsx")
...
<h2 style="color:#2c3e8c;margin-bottom:4px">&#128197; New Month of StatCan Border Crossing Data</h2>
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:13px">
  <tr><td style="padding:4px 10px;color:#666">Previously latest complete month</td><td style="padding:4px 10px">{prev_str}</td></tr>
  <tr><td style="padding:4px 10px;color:#666">Now latest complete month</td><td style="padding:4px 10px"><b>{new_str}</b></td></tr>
</table>
<p style="margin-top:20px">
  <a href="{REPO_BASE}/{STATCAN_XLSX}">&#128202; StatCan monthly spreadsheet</a>
  &nbsp;&nbsp;|&nbsp;&nbsp;
  <a href="{REPO_BASE}/{HARMONIZED_XLSX}">&#128260; Harmonized bidirectional spreadsheet</a>
  &nbsp;&nbsp;|&nbsp;&nbsp;
  <a href="{sc.TABLE_URL}">StatCan table 24-10-0057</a>
</p>
<p style="font-size:12px;color:#888;margin-top:24px">
  Detected from the Ambassador Bridge/DWT and Blue Water Bridge crossings
  reaching a full reported calendar month (GHIB is excluded from this
  check since it's too new to be a reliable bellwether).
</p>
</body></html>"""
...
def main():
    ...
    print(f"[{datetime.now(timezone.utc).isoformat()[:19]}Z] Checking StatCan Windsor-area data...")
    new_ym = sc.latest_complete_month()
    if new_ym is None:
        print("  Could not determine latest complete month.")
        return

    prev_ym = tuple(state["latest_ym"]) if state.get("latest_ym") else None
    print(f"  Latest complete month: {MONTH_NAMES[new_ym[1]-1]} {new_ym[0]} (previously: {prev_ym})")

    new_month_available = prev_ym is None or new_ym > prev_ym

    if new_month_available or force:
        os.makedirs(DATA_DIR, exist_ok=True)
        print("  Rebuilding StatCan monthly workbook...")
        sc.build_workbook(STATCAN_XLSX)
        ...
        send_email("[StatCan Alert] New month of border crossing data available", html, force=force)
    else:
        print("  No new complete month. Not emailing.")
